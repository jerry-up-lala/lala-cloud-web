    ...
    <a-card class="general-card" :title="$t('menu.system.role')">
      ...
      >
        ...
        <template #operate="{ record }">
          <a-button v-permission="['ROLE:INFO']" type="text">{{
...
<script lang="ts" setup>
   ...
   * 初始化查询条件
   */
  const queryModelDefault = (): RoleQueryParams => {
    return {
      roleName: '',
      description: '',
      createTimeRang: [],
  ...
  // 列表数据
  const tableData = ref<RoleResponseBody[]>([]);
  // 列表密度
  ...
  const tableDataRequest = async () => {
    ...
    try {
      const query: RolePageQueryParams = {
        current: tablePagination.current,
        pageSize: tablePagination.pageSize,
        ...queryModel.value,
      };
      const { data } = await rolePageApi(query);
  ...
  const jumpUpdate = (recordId: number) => {
    router.push({ name: 'RoleUpdate', query: { id: recordId } });
  };
  // =========== 跳转页面结束 ===========
